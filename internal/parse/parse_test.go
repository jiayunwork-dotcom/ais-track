...
func TestValidateMMSI(t *testing.T) {
	if err := ValidateMMSI("412000001"); err != nil {
		t.Fatalf("valid MMSI rejected: %v", err)
	}
	if err := ValidateMMSI("12"); err == nil {
		t.Fatal("short MMSI should fail")
	}
	if err := ValidateMMSI("912000001"); err == nil {
		t.Fatal("first digit 9 should fail")
	}
}

func TestRequireChronological(t *testing.T) {
	t0 := time.Date(2023, 1, 1, 0, 0, 0, 0, time.UTC)
	ok := []Record{
		{MMSI: "412000001", Timestamp: t0},
		{MMSI: "412000001", Timestamp: t0.Add(time.Minute)},
	}
	if err := RequireChronological(ok); err != nil {
		t.Fatal(err)
	}
	bad := []Record{
		{MMSI: "412000001", Timestamp: t0.Add(time.Minute)},
	...
	if err := RequireChronological(bad); err == nil {
		t.Fatal("expected chronological error")
	}
}
