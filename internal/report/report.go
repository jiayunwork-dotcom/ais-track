...
func Generate(groups map[string][]parse.Record, maxSOG float64, port []detect.Zone) *FleetReport {
	...
	for mmsi, track := range groups {
		...
		allAnoms = append(allAnoms, ca.Detect(live)...)

		ga := detect.DefaultGapAnomaly()
		allAnoms = append(allAnoms, ga.Detect(live)...)

		sj := detect.DefaultSpeedJump()
		allAnoms = append(allAnoms, sj.Detect(live)...)

		for _, z := range port {
			violations := detect.ZoneViolation(live, []detect.Zone{z})
